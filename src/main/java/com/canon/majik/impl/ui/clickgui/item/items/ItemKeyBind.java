...
public class ItemKeyBind extends Item<Module> {
    ...
    public int drawScreen(int mouseX, int mouseY, float partialTicks, int offset) {
        ...
        if(ClickGui.instance.cfont.getBoolean()){
            Initializer.CFont.drawString(pendingKey ? "PressKey..." : "Bind [" + Keyboard.getKeyName(getObject().getKey()) + "]",
                    x + 5,
                    y + height / 2f - mc.fontRenderer.FONT_HEIGHT / 2f,
                    -1);
        }else {
            mc.fontRenderer.drawStringWithShadow(pendingKey ? "PressKey..." : "Bind [" + Keyboard.getKeyName(getObject().getKey()) + "]",
                    x + 5,
                    y + height / 2f - mc.fontRenderer.FONT_HEIGHT / 2f,
                    -1);
        }
